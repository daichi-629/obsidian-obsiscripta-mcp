...
function resolveVaultLinks(
	...
	return markdown.replace(
		/(!)?\[\[([^\]\n]+)\]\]/g,
		(match, embedPrefix, rawLink: string) => {
			const { linkPath, subpath, displayText } =
				parseObsidianLink(rawLink);
			if (!linkPath) {
				return match;
			}

			const resolvedFile = context.app.metadataCache.getFirstLinkpathDest(
				linkPath,
				sourceFilePath,
			);
			if (!resolvedFile) {
...
function extractSectionContent(
	...
	if (matchingHeadings.length === 0) {
		...
		return {
			error: `Error: Section "${section}" with level ${level} not found.`,
		};
	}
	...
	const endLineIndex = nextSameLevelHeading
		? nextSameLevelHeading.lineIndex - 1
		: lines.length - 1;
	...
	const formatLine = (line: SectionLine): string =>
		withLineNumbers ? `${line.lineNumber}: ${line.text}` : line.text;
...
export const readNoteTool: MCPToolDefinition = {
	name: "read_note",
	description:
		"Read a section from a note by heading, excluding frontmatter.",
	inputSchema: {
		type: "object",
		properties: {
			path: {
			...
			level: {
				type: "number",
				description:
					"Heading level (1-6). If omitted, the first matching heading is used.",
			},
			mode: {
				type: "string",
			...
			include_subsections: {
				type: "boolean",
				description:
					"When true, include subsection headings/content. When false, subsection headings and their body content are excluded.",
				default: true,
			},
			max_chars: {
				type: "number",
				description:
					"Optional maximum number of characters in the returned content.",
			},
			with_line_numbers: {
				type: "boolean",
				description:
					"When true, prefixes each returned line with its markdown body line number.",
				default: false,
			},
		},
	...
	handler: async (args, context): Promise<MCPToolResult> => {
		...
		try {
			...
			const extracted = extractSectionContent(
				resolvedBody,
				section.trim(),
				level,
				mode,
